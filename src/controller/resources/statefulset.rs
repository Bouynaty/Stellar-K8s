//! StatefulSet builder for Validator nodes.
...
pub async fn ensure_statefulset(
    ...
    let mut sts = build_statefulset(node, enable_mtls);

    let base_labels = sts.metadata.labels.clone().unwrap_or_default();
    ...
    sts.metadata.labels = Some(final_labels);

    info!(statefulset = %name, "Applying StatefulSet");
    api.patch(&name, &patch_params(dry_run), &Patch::Apply(&sts))
        .await?;
    Ok(())
}

fn build_statefulset(node: &StellarNode, _enable_mtls: bool) -> StatefulSet {
