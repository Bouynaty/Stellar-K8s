...
impl FederationManager {
    ...
    pub async fn new(local_cluster_id: String, client: Client) -> Result<Arc<Self>, kube::Error> {
        ...
        clusters.insert(local_cluster_id.clone(), ClusterDefinition {
            id: local_cluster_id,
    ...
    pub async fn add_cluster(&self, cluster: ClusterDefinition) {
        ...
        info!("Added cluster to federation: {} ({})", cluster.name, cluster.id);
    }
    ...
    pub async fn start(&self) -> Result<(), anyhow::Error> {
        ...
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(StdDuration::from_secs(sync_config.sync_interval_seconds));
            loop {
                interval.tick().await;
        ...
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(StdDuration::from_secs(failover_config.health_check_interval_seconds));
            loop {
                interval.tick().await;
    ...
    async fn sync_resources(&self) -> Result<(), anyhow::Error> {
        ...
        for resource in &sync_config.resources {
            ...
            for cluster in clusters.values() {
                ...
                if let Err(e) = self.sync_resource_to_cluster(&primary_id, &cluster.id, resource).await {
                    error!("Failed to sync {} to cluster {}: {}", resource.kind, cluster.id, e);
                }
            }
        }
    ...
    async fn sync_resource_to_cluster(
        ...
        info!("Syncing {} from {} to {}", resource.kind, from_cluster, to_cluster);
        Ok(())
    }
    ...
    async fn check_local_health(&self) -> ClusterHealthStatus {
        ...
        match pod_api.list(&kube::api::ListParams::default().limit(1)).await {
    ...
    async fn update_cluster_health(&self, cluster_id: &str, new_status: ClusterHealthStatus) {
        ...
        if let Some(cluster) = clusters.get_mut(cluster_id) {
            let mut state = health_checks.entry(cluster_id.to_string()).or_insert(HealthCheckState {
    ...
    async fn check_failover(&self, config: &FailoverConfig) -> Result<(), anyhow::Error> {
        ...
        if let Some(primary_id) = primary_id {
            ...
            if let Some(state) = health_state {
                if state.consecutive_failures >= config.failure_threshold {
                    warn!("Primary cluster {} has {} consecutive failures, triggering failover", 
                        primary_id, state.consecutive_failures);
                    
                    if let Some(new_primary_id) = self.select_new_primary(&clusters, &primary_id).await {
        ...
        if config.auto_failback {
            for (cluster_id, state) in &health_checks {
                if !clusters.get(cluster_id).map(|c| c.is_primary).unwrap_or(false) {
                    if state.consecutive_successes >= config.recovery_threshold {
                        info!("Former primary {} recovered, considering failback", cluster_id);
                    }
                }
            }
        }
    ...
    async fn perform_failover(&self, old_primary: &str, new_primary: &str) -> Result<(), anyhow::Error> {
        ...
        self.send_failover_notification(old_primary, new_primary).await?;
    ...
    pub async fn get_status(&self) -> FederationStatus {
        ...
        for (id, cluster) in clusters.iter() {
            ...
            cluster_statuses.push(ClusterStatus {
                ...
                consecutive_successes: health.as_ref().map(|h| h.consecutive_successes).unwrap_or(0),
            });
        }
...
impl Default for SyncConfig {
    fn default() -> Self {
        Self {
            resources: vec![
                SyncResource { api_group: "".to_string(), api_version: "v1".to_string(), kind: "Secret".to_string(), enabled: true },
                SyncResource { api_group: "".to_string(), api_version: "v1".to_string(), kind: "ConfigMap".to_string(), enabled: true },
            ],
