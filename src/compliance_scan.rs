...
impl CisBenchmarkScanner {
    ...
    pub async fn scan(&self) -> Result<ComplianceScanResult, anyhow::Error> {
        ...
        // Store result
        self.results.write().await.insert(scan_id.clone(), result.clone());
        
        info!("CIS benchmark scan completed: {} - {} checks, {} passed, {} failed", 
            scan_id, summary.total_checks, summary.passed, summary.failed);
    ...
    async fn run_kube_bench(&self) -> Result<Vec<ComplianceFinding>, anyhow::Error> {
        ...
        for profile in &self.config.profiles {
            ...
            if output.status.success() {
                ...
                let findings = self.parse_kube_bench_output(&json_output, profile)?;
    ...
    fn parse_kube_bench_output(&self, json: &str, profile: &str) -> Result<Vec<ComplianceFinding>, anyhow::Error> {
        let mut findings = Vec::new();
        ...
        for control in output.controls {
            for test in control.tests {
                ...
                let finding = ComplianceFinding {
                    ...
                    references: vec![
                        format!("https://www.cisecurity.org/benchmark/{}", control.id),
                    ],
                    timestamp: Utc::now(),
                    framework: format!("CIS Kubernetes {}", control.id.split('.').next().unwrap_or("benchmark")),
    ...
    async fn evaluate_opa_policy(&self, policy_path: &str) -> Result<Vec<ComplianceFinding>, anyhow::Error> {
                        ...
                        for volume in volumes {
                            if volume.host_path.is_some() {
                                findings.push(ComplianceFinding {
                                    ...
                                    remediation: Some("Remove hostPath volumes or use projected volumes".to_string()),
                                    references: vec!["https://kubernetes.io/docs/concepts/storage/volumes/#hostpath".to_string()],
                                    timestamp: Utc::now(),
                                    framework: "Custom".to_string(),
    ...
    fn generate_summary(&self, findings: &[ComplianceFinding]) -> ScanSummary {
        ...
        for finding in findings {
            match finding.severity {
                ...
                Severity::Critical => *summary.by_severity.entry(Severity::Critical).or_insert(0) += 1,
            }
            
            *summary.by_framework.entry(finding.framework.clone()).or_insert(0) += 1;
    ...
    pub async fn get_latest_result(&self) -> Option<ComplianceScanResult> {
        self.results.read().await.values()
            .max_by_key(|r| r.started_at)
            .cloned()
    }

    /// Export scan result in specified format
    pub fn export_result(&self, result: &ComplianceScanResult, format: ReportFormat) -> Result<String, anyhow::Error> {
        match format {
    ...
    fn export_csv(&self, result: &ComplianceScanResult) -> Result<String, anyhow::Error> {
        ...
        for finding in &result.findings {
            let resource_kind = finding.resource.as_ref().map(|r| r.kind.as_str()).unwrap_or("");
            let resource_name = finding.resource.as_ref().map(|r| r.name.as_str()).unwrap_or("");
            let resource_namespace = finding.resource.as_ref().and_then(|r| r.namespace.as_deref()).unwrap_or("");
            ...
            csv.push_str(&format!(
                ...
                resource_kind,
                finding.resource.as_ref().map(|r| r.name.as_str()).unwrap_or(""),
                resource_namespace,
                finding.remediation.as_deref().unwrap_or("").replace(',', ";"),
    ...
    fn export_sarif(&self, result: &ComplianceScanResult) -> Result<String, anyhow::Error> {
        ...
        let rules: Vec<SarifRule> = result.findings.iter().map(|f| SarifRule {
            id: f.check_id.clone(),
            name: f.check_id.clone(),
            short_description: SarifDescription { text: f.description.clone() },
            full_description: SarifDescription { text: f.description.clone() },
            default_configuration: SarifConfig { level: format!("{:?}", f.severity).to_lowercase() },
        }).collect();
        
        let results: Vec<SarifResult> = result.findings.iter().map(|f| SarifResult {
            ...
            message: SarifMessage { text: f.description.clone() },
            locations: f.resource.as_ref().map(|r| vec![SarifLocation {
                physical_location: SarifPhysicalLocation {
                    artifact_location: SarifArtifactLocation {
...
impl ComplianceMonitor {
    ...
    pub async fn start(&self) {
        let interval = tokio::time::interval(Duration::from_secs(self.config.scan_interval_seconds));
        ...
        loop {
            ...
            match self.scanner.scan().await {
                Ok(result) => {
                    info!("Scheduled scan completed: {} checks, {} failed", 
